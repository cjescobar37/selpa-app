-- Corrective migration: public.tournaments.status is text in the deployed schema.
-- Preserve the PAUSED contract without changing the column type or lifecycle semantics.
...
begin
  ...
  if v.status <> 'OPEN' then raise exception 'INVALID_STATUS_TRANSITION' using errcode = '23514'; end if;
  update public.tournaments set status = 'PAUSED', updated_at = now() where id = v.id returning * into v;
  return v;
end;
$$;
...
begin
  ...
  select * into v from public.tournaments where id = p_tournament_id and club_id = p_club_id for update;
  if not found then raise exception 'TOURNAMENT_NOT_FOUND' using errcode = 'P0002'; end if;
  if v.status <> 'PAUSED' then raise exception 'INVALID_STATUS_TRANSITION' using errcode = '23514'; end if;
  update public.tournaments set status = 'OPEN', updated_at = now() where id = v.id returning * into v;
  return v;
end;
$$;
...
as $$
declare v_me uuid:=auth.uid();v_team_id uuid;v_reg_id uuid;v_exists int;v_status text;
begin
