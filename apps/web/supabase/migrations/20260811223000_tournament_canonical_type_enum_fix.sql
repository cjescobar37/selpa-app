...
-- Stage 5A.8A corrective migration: public.tournaments.type is the enum
-- public.tournament_type, while public.tournaments.tournament_type is text.
do $$
begin
  if to_regprocedure('public.create_tournament_canonical(uuid,jsonb)') is null then
    raise exception 'La corrección requiere Stage 5A.8A aplicado.';
  end if;
end;
$$;
...
declare actor uuid:=auth.uid(); p jsonb:=coalesce(p_payload,'{}'::jsonb); rules jsonb; result public.tournaments%rowtype;
  name text; kind public.tournament_type; gender text; segment text; category_rule text; category_id integer; sum_target integer; age_id uuid;
