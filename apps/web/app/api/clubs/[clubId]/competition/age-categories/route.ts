...
import { isUuid, validateAgeCategoryInput } from '@/features/competition/catalogs/competition-catalogs.validation'
...
export async function GET(req: NextRequest, context: RouteContext) {
  ...
  try {
    return NextResponse.json({ ageCategories: await listAgeCategories(clubId) })
  } catch (error: unknown) {
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  const validation = validateAgeCategoryInput(await req.json().catch(() => null))
  if ('error' in validation) return NextResponse.json({ error: validation.error }, { status: 400 })

  try {
    const ageCategory = await createAgeCategory(clubId, auth.user.id, validation.value)
    return NextResponse.json({ ok: true, ageCategory }, { status: 201 })
  } catch (error: unknown) {
    return competitionCatalogErrorResponse(error)
  }
}
...
export async function PATCH(req: NextRequest, context: RouteContext) {
  ...
  if (!body || typeof body !== 'object' || Array.isArray(body) || !isUuid((body as Record<string, unknown>).id)) {
    return NextResponse.json({ error: 'La categoría de edad es inválida.' }, { status: 400 })
  }

  const id = String((body as Record<string, unknown>).id)
  try {
    const current = await getAgeCategory(clubId, id)
    const validation = validateAgeCategoryInput({ ...current, ...body })
    if ('error' in validation) return NextResponse.json({ error: validation.error }, { status: 400 })
    const ageCategory = await updateAgeCategory(clubId, id, validation.value)
    return NextResponse.json({ ok: true, ageCategory })
  } catch (error: unknown) {
    return competitionCatalogErrorResponse(error)
  }
}
