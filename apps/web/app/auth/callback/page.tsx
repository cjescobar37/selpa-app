...
export default function AuthCallbackPage() {
  ...
  const nextPath = useMemo(() => searchParams.get('next') || '/auth/post-login', [searchParams])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (errorDescription) {
          if (!active) return
          setAlert({ variant: 'error', title: 'No se pudo completar el acceso', message: errorDescription })
          return
        }
        ...
        if (code) {
          ...
          if (error) {
            if (!active) return
            setAlert({ variant: 'error', title: 'No se pudo crear la sesión', message: error.message })
            return
          }
        }
        ...
        if (!active) return
        setAlert({ variant: 'error', title: 'Falló la validación', message: e?.message ?? 'Error inesperado.' })
      }
    })()
      ...
      <div className="px-authCard">
        <div className="px-authTop">
          <div className="px-authBrand">
            ...
            <div className="px-authBrandText">
              ...
              <p className="px-authSub">Estamos cerrando el proceso de autenticación.</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-authBody">
          ...
          <div className="px-authRow">
            <Link className="px-link" href="/login">Volver a login</Link>
          </div>
        </div>
      </div>
    </div>
