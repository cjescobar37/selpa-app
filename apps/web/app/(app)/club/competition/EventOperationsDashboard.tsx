...
export default function EventOperationsDashboard({ seriesId, eventId }: { seriesId: string; eventId: string }) {
  ...
  const primaryAction = detail.allowed_actions.edit ? { label: 'Configurar', href: `/club/competition/series/${seriesId}?event=${eventId}` } : detail.allowed_actions.schedule ? { label: 'Programar', href: `/club/competition/series/${seriesId}?event=${eventId}` } : null

  return <main className={styles.page}>
    <header className={styles.hero}><div className={styles.eyebrow}><Link href={`/club/competition/series/${seriesId}`} aria-label="Volver"><ArrowLeft size={18} /></Link><span>OPERACIÓN DE FECHA</span><b className={`${styles.badge} ${styles[`status_${event.status}`]}`}>{labels[event.status]}</b></div><div className={styles.title}><div><h1>{event.name}</h1><p><CalendarDays size={14} />{event.planned_starts_at ? new Date(event.planned_starts_at).toLocaleString('es-AR', { dateStyle: 'medium', timeStyle: 'short' }) : 'Sin fecha'}{event.venue_name ? <><MapPin size={14} />{event.venue_name}</> : null}</p></div>{primaryAction ? <Link className={styles.primary} href={primaryAction.href}>{primaryAction.label}</Link> : null}</div><div className={styles.meta}><span>{eventTypes[event.event_type]}</span><span>{event.is_public ? 'Pública' : 'Privada'}</span><span>Rev. {event.revision}</span></div></header>
    ...
    <section className={styles.pending}><span><b>{summary.pendingHomologations}</b> homologaciones</span><span><b>{summary.pendingSettlements}</b> settlements</span><span><b>{summary.pendingPublications}</b> publicaciones</span></section>
    ...
    <section className={styles.timeline}><div className={styles.sectionTitle}><div><span>TRAZABILIDAD</span><h2>Timeline operativo</h2></div></div>{[
      ['Fecha creada', true], ['Torneo vinculado', summary.linked > 0], ['Resultados completados', event.status === 'COMPLETED'], ['Homologación', activeDivisions.length > 0 && summary.pendingHomologations === 0], ['Settlement', activeDivisions.length > 0 && summary.pendingSettlements === 0 && Object.values(operations).some(item => item.settlements.length)], ['Publicación', activeDivisions.length > 0 && summary.pendingPublications === 0 && Object.values(operations).some(item => item.settlements.some(row => status(row) === 'PUBLISHED'))], ['Ranking actualizado', Object.values(operations).some(item => item.settlements.some(row => status(row) === 'PUBLISHED'))],
