...
export default function CompetitionAdmin({ screen, mode = 'hub' }: { screen: Screen; mode?: 'hub' | 'circuits' }) {
  ...
  if (screen.kind === 'detail' && detail) {
      ...
      : focusAction && eventOperationsHref && focusAction.key === 'CLOSE_DATE'
        ...
        : focusAction && homologationHref && ['REVIEW_RESULTS', 'APPROVE_RESULTS', 'PUBLISH_POINTS'].includes(focusAction.key)
          ...
          : focusState?.key === 'SETTLED'
            ? { href: nextEventHref ?? primaryHref, label: 'Operar próxima fecha' }
            : null
    const primaryActionHref = pipelineAction?.href ?? nextEventHref ?? primaryHref
    const primaryActionLabel = pipelineAction?.label ?? (nextEventHref ? 'Operar próxima fecha' : hasEvents ? 'Agregar fecha' : 'Agregar primera fecha')
    ...
    return <div className={styles.page}>
      ...
      {detailTab === 'dates' ? <section className={styles.controlPanel}><div className={styles.sectionHead}><span>AGENDA</span><h2>{hasEvents ? 'Fechas del circuito' : 'Todavía no hay fechas'}</h2><p>{hasEvents ? 'Operá la fecha o entrá directamente al centro de control del torneo.' : 'Agregá el primer torneo para empezar la agenda.'}</p></div>{primaryHref ? <Link className={styles.addDateAction} href={primaryHref}>+ {hasEvents ? 'Agregar fecha' : 'Agregar primera fecha'}</Link> : null}{hasEvents ? <div className={styles.dateList}>{events.map((event, index) => { const state = deriveCompetitionEventOperationalState(event); const operationsHref = `/club/competition/series/${item.id}/events/${event.id}`; return <article key={event.id} className={styles.dateRow}><span className={`${styles.operationalBadge} ${styles[`tone_${state.tone}`]}`}>{state.label}</span><small>FECHA {index + 1}</small><strong>{event.name}</strong><p>{formatEventSportDate(event.tournament_starts_at ?? event.planned_starts_at, event.tournament_ends_at ?? event.planned_ends_at)}{event.venue_name ? ` · ${event.venue_name}` : ''}</p><div className={styles.dateActions}><Link href={operationsHref}>Operar fecha</Link>{event.tournament_id ? <Link className={styles.dateTournamentAction} href={`/club/torneos/${event.tournament_id}`}>Centro de control <ChevronRight size={14} /></Link> : null}</div></article> })}</div> : null}{searchParams.get('event') ? <SeriesEventsAdmin clubId={clubId} series={detail} events={events} request={api} reload={() => load({ silent: true })} hideCreate hideList /> : null}</section> : null}
