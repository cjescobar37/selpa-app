...
import { ArrowLeft, ArrowRight, CalendarDays, Check, ChevronRight, LoaderCircle, Pencil, Plus, Trash2, Trophy, X } from 'lucide-react'
...
export default function SeriesCreateWizard({ clubId, request }: { clubId: string; request: Request }) {
  ...
  const [pointsPreviewOpen,setPointsPreviewOpen]=useState(false)
  ...
  return <section className={styles.wizard}>
    ...
    <main>
      ...
      {step === 2 ? <div className={styles.stepBody}>
        ...
        <section className={`${styles.secondaryCard} ${styles.pointsCard}`}><Trophy size={20}/><label><span>Tabla de puntos</span><select value={form.schemeId} onChange={(event) => update('schemeId', event.target.value)}><option value="">Elegir tabla</option>{schemes.map((item) => <option value={item.id} key={item.id}>{schemeLabel(item)}</option>)}</select></label>{selectedScheme ? <div className={styles.pointsSummary}><strong>{schemeLabel(selectedScheme)}</strong>{schemeRules.slice(0, 3).map((rule) => <span key={rule.id}>{pointLabels[rule.rule_key]} <b>{rule.points} pts</b></span>)}<button type="button" onClick={()=>setPointsPreviewOpen(true)}>Ver tabla</button></div> : null}<button className={styles.createPointsLink} type="button" onClick={()=>router.push('/club/competition/points-schemes?returnTo=%2Fclub%2Fcompetition%2Fseries%2Fnew')}>+ Crear tabla de puntos</button></section>
        ...
        {pointsPreviewOpen&&selectedScheme?<div className={styles.sheetBackdrop} role="presentation" onMouseDown={()=>setPointsPreviewOpen(false)}><section className={styles.pointsSheet} role="dialog" aria-modal="true" aria-label="Tabla de puntos" onMouseDown={event=>event.stopPropagation()}><header><div><small>TABLA DE PUNTOS</small><h2>{schemeLabel(selectedScheme)}</h2></div><button type="button" aria-label="Cerrar" onClick={()=>setPointsPreviewOpen(false)}><X size={18}/></button></header>{schemeRules.map(rule=><div key={rule.id}><span>{pointLabels[rule.rule_key]}</span><strong>{rule.points} pts</strong></div>)}</section></div>:null}
      </div> : null}
