...
export default function PlayerAdministrationPage() {
  ...
  if (!detail) return <main className={styles.page}><Link href="/club/jugadores" className={styles.back}><ArrowLeft size={17} />Jugadores</Link>{error ? <p className={styles.error}>{error}</p> : <SelpaLoader title="Preparando ficha" subtitle="" />}</main>
