...
export type PdfGroup = { name: string; rows: { name: string; seed: number | null; qualified: boolean; metrics: number[] }[]; matches: PdfMatch[] }
...
type StandingsInput = { group: { id: string }; qualifiers: { team_id: string }[]; standings: { team_id: string; played: number; wins: number; losses: number; match_points: number; set_difference: number; game_difference: number }[] }
