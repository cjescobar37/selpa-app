...
import { buildLocalPreview, getClubInitials, uploadClubLogo } from '@/lib/clubAssets'
...
export default function PlatformCreateClubPage() {
  ...
  const displayLogo = useMemo(() => logoPreview || v.logo_url || '', [logoPreview, v.logo_url])
  const onChange = (k: keyof typeof initial, value: string) => setV((p) => ({ ...p, [k]: value }))

  async function onLogoFileChange(file: File | null) {
    if (!file) return

    const local = buildLocalPreview(file)
    setLogoPreview(local)
    setUploadingLogo(true)
    setAlert({ variant: 'info', title: 'Subiendo logo…', message: 'Estamos guardando el logo en Supabase Storage.' })

    try {
      const uploaded = await uploadClubLogo({ file, folder: 'platform' })
      setV((prev) => ({ ...prev, logo_url: uploaded.publicUrl }))
      setAlert({ variant: 'success', title: 'Logo listo', message: 'El logo ya quedó vinculado al alta del club.' })
    } catch (err: any) {
      setAlert({
        variant: 'warning',
        title: 'No pude subir el logo automáticamente',
        message: err?.message ?? 'Podés continuar pegando la URL manualmente.',
      })
    } finally {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch('/api/platform/create-club', {
        ...
        body: JSON.stringify({
          ...
          club: {
            ...
            courts_surface: v.courts_surface,
            logo_url: v.logo_url,
            notes: v.notes,
            rules_pdf_url: v.rules_pdf_url,
          },
      ...
      setAlert({
        variant: 'success',
        title: 'Club creado',
        message: `Club: ${json.clubName}. OWNER: ${v.owner_email}. Slug generado: ${json.slug}.`,
      })
      ...
      <div className="px-platform">
        <div className="px-platformHead">
          <div>
            ...
            <div className="px-platformSub">Crea el club, deja el branding listo y genera el usuario OWNER.</div>
          </div>
        </div>
        ...
        <form onSubmit={onSubmit} style={{ marginTop: 14, display: 'grid', gap: 12 }}>
          <div className="px-card px-glass" style={{ borderRadius: 18 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px,280px) 1fr', gap: 16, marginTop: 12 }}>
              <div style={{ display: 'grid', gap: 10, alignContent: 'start' }}>
                <div className="px-label">Logo del club</div>
                <label
                  style={{
                ...
                >
                  <span
                    ...
                    style={{ width: 96, height: 96, borderRadius: 24, fontSize: 30, background: 'rgba(81,211,225,.22)' }}
                  >
                    {displayLogo ? <img src={displayLogo} alt="Logo del club" /> : <span>{getClubInitials(v.name || 'Club')}</span>}
                  </span>
                  <span className="px-help" style={{ textAlign: 'center' }}>
                    {uploadingLogo ? 'Subiendo logo…' : 'Elegí una imagen. Si el bucket no existe, podés dejar una URL manual.'}
                  </span>
                  <input type="file" accept="image/*" onChange={(e) => onLogoFileChange(e.target.files?.[0] ?? null)} style={{ display: 'none' }} />
                </label>
                <div className="px-field">
                  <label className="px-label">URL logo</label>
          ...
          <div className="px-card px-glass" style={{ borderRadius: 18 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 10 }}>
              ...
              <div className="px-field"><label className="px-label">Clave *</label><input className="px-input" type="password" value={v.owner_password} onChange={e => onChange('owner_password', e.target.value)} required minLength={8} /></div>
            </div>
          </div>

          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            <button className="px-btn px-btn--ghost" type="button" onClick={() => router.push('/platform/clubs')}>
              Cancelar
            </button>
            <button className="px-btn" type="submit" disabled={loading || uploadingLogo}>
