...
export function validateEventPatch(input:unknown){const b=record(input);if(!b)return null;if(b.event_type&&!EVENT_TYPES.includes(String(b.event_type).toUpperCase() as never))return null;if('timezone'in b){const timezone=validCompetitionTimezone(b.timezone);if(!timezone)return null;return {...b,timezone}}return b}
