...
const ERROR_RESPONSES: Record<ClubInviteErrorCode, { status: number; message: string }> = {
  ...
  staff_already_exists: { status: 409, message: 'Ese usuario ya forma parte del equipo del club.' },
  owner_already_exists: { status: 409, message: 'El OWNER actual no puede modificarse mediante invitaciones.' },
  player_not_eligible: { status: 409, message: 'Ese usuario ya no está disponible para ser promovido.' },
  concurrent_update: { status: 409, message: 'El equipo cambió mientras realizabas la acción. Intentá nuevamente.' },
